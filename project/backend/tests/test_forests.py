...
def test_read_forests(test_app_with_db):
    ...
    assert (
        len(list(filter(lambda forest: forest["id"] == forest_id, forests["items"]))) == 1
    )
...
def test_read_forest_with_forest_type_filter(test_app_with_db):
    ...
    # test that we do not get an item back in the response considering it should be filtered
    assert not list(filter(lambda forest: forest["type"] == filter_type, response["items"]))
...
def test_read_forest(test_app_with_db):
    ...
    # test that the geojson properties uploaded match the ones returned from the db / api
...
def test_read_forest_doesnt_exist(test_app_with_db):
    ...
    assert response.json() == {"detail": f"Forest record '{incorrect_forest_id}' not found"}